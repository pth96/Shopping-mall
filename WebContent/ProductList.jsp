...
<body>
	...
	<c:forEach var="product" items="${productlist}">
		...
		<div class="main">
			<a href="productview?pid=${product.pid}"><img
				src="Uploadfile/${product.pfilename}" width="200" height="200"></a>
			<br> <a href="productview?pid=${product.pid}">${product.pname}</a>
			<br> ${product.pprice}원<br>
		</div>
		<c:if test="${i%j == j-1 }">
			<br>
		</c:if>
	...
	[이전]&nbsp;
		...
		<c:if test="${paging.page>1}">
			<a href="productlist?page=${paging.page-1}&pgender=${pgender}">[이전]</a>&nbsp;
	</c:if>
		...
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i"
			...
			${i}
				...
				<c:otherwise>
					<a href="productlist?page=${i}&pgender=${pgender}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:if test="${paging.page<paging.maxPage}">
			<a href="productlist?page=${paging.page+1}&pgender=${pgender}">[다음]</a>
		</c:if>
	</div>
